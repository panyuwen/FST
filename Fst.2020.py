...
## Note:
## python2.7
## plink1.9 is required, should be called by typing "plink"
...
def getgroup(infofile,pairfile):
    ...
    if pairfile == 'NO':
        ...
        pairlist = list(pairdf.values)
        pairlist = [tuple(x) for x in pairlist]
    ...
    for group in grouplist:
        while ((not os.path.isfile(outprefix+'_freqs/'+group+'.info')) | (not os.path.isfile(outprefix+'_freqs/'+group+'.frq.gz'))):
            ...
            if regiondf.shape[0] == 0:
                subprocess.call('plink --bfile '+bfile+' --keep '+outprefix+'_freqs/'+group+'.info --freq gz --out '+outprefix+'_freqs/'+group,shell=True)
            else:
                subprocess.call('plink --bfile '+bfile+' --keep '+outprefix+'_freqs/'+group+'.info --extract range '+outprefix+'.region --freq gz --out '+outprefix+'_freqs/'+group,shell=True)
            time.sleep(1)
